...
def cosReadData(Des3_Cipher, address_int, length_int, package_size):
  command_bytes = [0xA2,0xA4,0x00,0x0C,0x02,0xDA,0x01]
  sendCommand(command_bytes)
  data_bytes = iso14443a4aReadBinary(0xA2, address_int, length_int, package_size)
  print(data_bytes)


def iso14443a4aReadBinary(cla_bytes, address_start_int, length_int, RF_PACKAGE_SIZE):
  data_read_bytes = b''
  ...
  for i in range (0, package_number_int):
    for retry_cnt_int in range (0, 10):
      ...
      if s1 == 144 and s2 == 0:
        ...
        data_read_bytes = data_read_bytes + res[1:1+package_size_int]
        break
    if retry_cnt_int==9:
      return b''
...
def ISO14443_4A_ReadBinary(sscom, cla_bytes, address_start_int, length_int, RF_PACKAGE_SIZE):
  ...
  for i in range (0, package_number_int):
    for retry_cnt_int in range (0, 10):
      ...
      des3_key_int_list[1] = int(data_address_int%256)
      data_address_bytes = int_list2bytes(des3_key_int_list)
      rf_command_bytes = iso14443a_add_crc16(b'\x02' + cla_bytes + b'\xB0' + data_address_bytes + bytes([package_size_int]))
  ...
  if package_bytes_left_int!=0:
    ...
    for retry_cnt_int in range (0, 10):
      ...
      des3_key_int_list[1] = int(data_address_int%256)
      data_address_bytes = int_list2bytes(des3_key_int_list)
      rf_command_bytes = iso14443a_add_crc16(b'\x02' + cla_bytes + b'\xB0' + data_address_bytes + bytes([package_bytes_left_int]))
      command_bytes = build_com_command(rf_command_bytes)
      data_bytes, length = sscom_transceive_bytes(sscom, command_bytes)
      if data_bytes[-4:-2] != b'\x90\x00':
        continue
